...
gulp.task('webpack', function(){
    gulp.src('./src/fund-party-20151116.html').pipe(gulp.dest('./prebuild'));
    return gulp.src('./src/js/fund-party-20151116.js')
...
gulp.task('replace', ['css', 'js', 'img'], function(){
    var mapFile = gulp.src("./build/rev-manifest.json");
    return gulp.src('./prebuild/fund-party-20151116.html')
        .pipe(plugins.revReplace({manifest: mapFile}))
        .pipe(gulp.dest('build'));
});
...
gulp.task('deploy', ['build'], function(){
    gulp.src('./build/*')
        .pipe(gulp.dest('../fund-party-20151116'));
});
